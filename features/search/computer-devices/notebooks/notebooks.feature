...
@customfield
  Сценарий: Фильтр по типу процессора
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип процессора" равно "AMD"
...
@customfield
  Сценарий: Фильтр по типу видеокарты
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип видеокарты" равно "встроенная"
...
@bugs @bug34298
  Сценарий: Фильтр по времени работы от батареи
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Время работы от аккумулятора" >= "2"
       И в деталях первого объявления "Время работы от аккумулятора" <= "10"
...
@bugs @bug34298
  Сценарий: Фильтр по времени работы от батареи
   Когда я делаю поиск по следующим параметрам:
      | parameter               | max     |
      | Время работы от батареи | 1000000 |
      И на странице поиска загружен список результатов
