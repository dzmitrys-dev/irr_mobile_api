# language:ru
# encoding: utf-8
Функционал: Компьютерная техника -> Flash-накопители -> Карты памяти

  Контекст:
    Когда открыта страница для города "Москва"
        И на главной странице я перехожу в категорию "Компьютерная техника -> Flash-накопители -> Карты памяти"
...
@customfield
  Сценарий: Фильтр по марке
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Марка" равно "Kingston"

@customfield
  Сценарий: Фильтр по типу карты
   Когда я делаю поиск по следующим параметрам:
      | parameter | value     |
      | Тип карты | microSDHC |
      И на странице поиска загружен список результатов
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип карты" равно "microSDHC"
...
@bugs @bug35004
  Сценарий: Фильтр по объему памяти
   Когда я делаю поиск по следующим параметрам:
      | parameter    | min | max  |
      | Объем памяти | 512 | 2048 |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Объем памяти" >= "512"
       И в деталях первого объявления "Объем памяти" <= "2048"
