# language:ru
# encoding: utf-8
Функционал: Товары для детей -> Детская мебель -> Кроватки

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Товары для детей -> Детская мебель -> Кроватки"
...
@customfield
  Сценарий: Фильтр по производителю
   Когда я делаю поиск по следующим параметрам:
      | parameter     | value    |
      | Производитель | Papaloni |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Производитель" равно "Papaloni"
