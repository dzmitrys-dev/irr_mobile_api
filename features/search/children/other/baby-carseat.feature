# language:ru
# encoding: utf-8
Функционал: Товары для детей -> Автокресла

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Товары для детей -> Автокресла"
...
@customfield
  Сценарий: Фильтр по группе
   Когда я делаю поиск по следующим параметрам:
      | parameter | value                    |
      | Группа    | от 9 до 18 кг (группа 1) |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Группа" равно "от 9 до 18 кг (группа 1)"
...
@customfield
  Сценарий: Фильтр по марке и модели
   Когда я делаю поиск по следующим параметрам:
      | parameter | value                 |
      | Марка     | Romer                 |
      | Модель    | Baby-Safe Plus Isofix |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Марка" равно "Romer"
       И в деталях первого объявления "Модель" равно "Baby-Safe Plus Isofix"
