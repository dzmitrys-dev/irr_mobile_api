# language:ru
# encoding: utf-8
Функционал: Строительство и ремонт -> Электрика -> Прочее

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Строительство и ремонт -> Электрика -> Прочее"
...
@all_results
  ...
  Сценарий: Фильтр по типу электрики
   Когда я делаю поиск по следующим параметрам:
      | parameter     | value              |
      | Тип электрики | автотрансформаторы |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип электрики" равно "автотрансформаторы"
