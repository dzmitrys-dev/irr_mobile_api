...
@customfield
  Сценарий: Фильтр по типу кузова
    Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип прицепа" равно "Низкорамный трал"
...
@customfield
  Сценарий: Фильтр по марке и модели
   ...
   Когда на странице поиска я запоминаю следующие поля из деталей объявления:
       ...
       | Марка  |
       | Модель |
      То в результатах поиска присутствует объявление, у которого "Марка" равно "Schmitz"
       И в результатах поиска присутствует объявление, у которого "Модель" равно "SKO"
       И в результатах поиска присутствует объявление, у которого "Марка" равно "Grunwald"
