# language:ru
# encoding: utf-8
Функционал: Электроника и техника -> Техника для авто -> Устройства громкой связи

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Электроника и техника -> Техника для авто -> Устройства громкой связи"
   ...
   Когда я раскрываю список фильтров
       * в списке фильтров показаны следующие фильтры:
         ...
         | Цена            |
          | Марка |
| Тип |
| Источник питания спикерфона |
| Дисплей |
| Возможность подключения к штатной акустике |
| Подключение USB |
| Bluetooth |
         | Ключевые слова  |
         | С фото          |
...
@customfield
  Сценарий: Фильтр по марке
    Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Марка" равно "Nokia"
...
@customfield
  Сценарий: Фильтр по типу
    Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип" равно "спикерфон"

@customfield
@empty_results
  Сценарий: Фильтр по источнику питания спикерфона
    Когда я делаю поиск по следующим параметрам:
      | parameter                   | value                  |
      | Источник питания спикерфона | встроенный аккумулятор |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Источник питания спикерфона" равно "встроенный аккумулятор"
...
@customfield
@empty_results
  Сценарий: Фильтр по возможности подключения к штатной акустике
    Когда я делаю поиск по следующим параметрам:
      | parameter                                  | value |
      | Возможность подключения к штатной акустике | x     |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления присутствует "Возможность подключения к штатной акустике"
