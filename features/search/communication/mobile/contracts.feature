# language:ru
# encoding: utf-8
Функционал: Телефоны и связь -> Мобильная связь -> Контракты

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Телефоны и связь -> Мобильная связь -> Контракты"
...
@customfield
  Сценарий: Фильтр по оператору связи
   Когда я делаю поиск по следующим параметрам:
      | parameter      | value |
      | Оператор связи | МТС   |
      И на странице поиска загружен список результатов
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Оператор связи" равно "МТС"

@customfield
  Сценарий: Фильтр по название тарифа
   Когда я делаю поиск по следующим параметрам:
      | parameter       | value    |
      | Название тарифа | Безлимит |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Название тарифа" равно "Безлимит"
