# language:ru
# encoding: utf-8
Функционал: Спорт, туризм, отдых -> Фитнес, атлетика, борьба -> Бокс, товары для единоборств

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Спорт, туризм, отдых -> Фитнес, атлетика, борьба -> Бокс, товары для единоборств"
...
@customfield
  Сценарий: Фильтр по типу снаряжения
   Когда я делаю поиск по следующим параметрам:
      | parameter      | value                      |
      | Тип снаряжения | экипировка для единоборств |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип снаряжения" равно "экипировка для единоборств"
