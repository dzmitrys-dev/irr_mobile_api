# language:ru
# encoding: utf-8
Функционал: Спорт, туризм, отдых -> Велосипеды

  Контекст:
    Когда открыта страница для региона "Россия"
        И на главной странице я перехожу в категорию "Спорт, туризм, отдых -> Велосипеды"
...
@all_results
  ...
  Сценарий: Фильтр по типу товара
   Когда я делаю поиск по следующим параметрам:
      | parameter  | value |
      | Тип товара | новый |
      И на странице поиска загружен список результатов
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип товара" равно "новый"
...
@customfield
  Сценарий: Фильтр по производителю
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Производитель" равно "STELS"

@customfield
  Сценарий: Фильтр по типу велосипеда
   Когда я делаю поиск по следующим параметрам:
      | parameter      | value      |
      | Тип велосипеда | спортивные |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип велосипеда" равно "спортивные"
