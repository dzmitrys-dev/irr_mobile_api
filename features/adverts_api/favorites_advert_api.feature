...
Функционал: Избранные объявления
  ...
  Сценарий: Добавление объявления в избранное
    Когда я вхожу под пользователем с ролью "Обычный пользователь 1" посредством API
      ...
      * я добавляю объявление в избранное
      * я получаю список избранных объявлений
      * я проверяю, что в списке избранных объявлений есть объявление с таким идентификатором
      
  Сценарий: Удаление объявления из избранного
    Когда я вхожу под пользователем с ролью "Обычный пользователь 1" посредством API
      То я запоминаю авторизационный токен
      * я подаю объявление со следующими параметрами:
        | parameter                  | value                           |
      ...
      * я получаю список избранных объявлений
      * я проверяю, что в списке избранных объявлений есть объявление с таким идентификатором
      * я удаляю объявление из избранного
      * я проверяю, что в списке избранных объявлений нет объявления с таким идентификатором